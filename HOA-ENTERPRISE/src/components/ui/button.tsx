...
export interface ButtonProps
  ...
  asChild?: boolean;
  /**
   * When true the button shows a spinning loader prefix and is disabled. The
   * caller still controls the label so e.g. `<Button loading>Save</Button>`
   * keeps the text but adds the spinner. Use this on mutation buttons so the
   * user always sees something is happening.
   */
  loading?: boolean;
}
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, disabled, children, ...props }, ref) => {
    ...
    // When asChild is used, we cannot inject a spinner without disturbing the
    // Slot's single-child contract. Fall back to plain disabled passthrough.
    if (asChild) {
      return (
        <Comp
