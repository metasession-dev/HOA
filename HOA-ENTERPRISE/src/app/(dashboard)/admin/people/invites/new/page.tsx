...
export default function NewResidentInvitePage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/admin/people/invites');
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <Link href="/admin/people/invites" className="inline-flex items-center gap-1 text-caption text-muted-foreground hover:text-graphite">
        <ChevronLeft className="h-3 w-3" />Resident invites
      </Link>
              ...
              <>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-1.5">
                    ...
                    {!resident.email && (
                      <p className="text-caption text-muted-foreground">
                        This person doesn&rsquo;t have an email on file — add one here, or update them in /admin/people first.
                      </p>
                    )}
                  </div>
                  <div className="space-y-1.5">
                ...
                <label className="flex items-start gap-2 rounded-lg bg-stone-surface/40 p-3 text-caption text-graphite">
                  ...
                  <span>
                    ...
                    <span className="block text-muted-foreground">
                      Tick this if the resident is also on the board / exco and needs access to /admin. Most residents shouldn&rsquo;t.
                    </span>
                  </span>
                </label>
              </>
