...
/**
 * Topbar role switcher (resident PWA flavour). Mirrors the Enterprise app's
 * version — the only behavioural difference is `inResidentApp = true`, which
 * inverts the cross-app handoff direction.
 *
 * Apps:
...
export function RoleSwitcher() {
  ...
  const handleSwitch = async (role: string, orgId: string) => {
    ...
    try {
      ...
      const inResidentApp = true;

      if (targetsResidentApp !== inResidentApp) {
        const residentBase =
          process.env.NEXT_PUBLIC_RESIDENTS_URL || window.location.origin;
        const enterpriseBase =
          process.env.NEXT_PUBLIC_ENTERPRISE_URL || 'http://localhost:3002';
      ...
      {open && (
        ...
        >
          ...
          <p className="border-t border-stone-surface px-3 py-2 text-[11px] text-muted-foreground">
            Switching to an admin role opens the management console.
          </p>
        </div>
      )}
    </div>
