...
import { ChevronLeft, Ban, Car, User2, Calendar, Clock, Building2 } from 'lucide-react';
...
export default function AdminPassDetailPage() {
  ...
  const handleRevoke = async () => {
    const ok = await confirm({
      ...
      description: 'The QR will stop working immediately and the resident will be notified.',
                ...
                <li key={log.id} className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <p className="text-caption text-muted-foreground">
                      ...
                      {log.overrideReason && ` · Reason: ${log.overrideReason}`}
                    </p>
                  </div>
                  {log.overrideReason && <Badge variant="warning">Override</Badge>}
                </li>
              ))}
            </ul>
          </CardContent>
